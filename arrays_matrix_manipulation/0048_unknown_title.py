...
- Transposing the matrix takes O(n^2) time, as we iterate over all elements above the diagonal.
- Reversing each row takes O(n^2) time, as we reverse n rows, each of length n.
- Overall time complexity: O(n^2).

Space Complexity Analysis:
- The rotation is done in-place, so no additional space is used apart from a few variables.
