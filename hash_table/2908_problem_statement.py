"""
LeetCode Problem #2908: Problem Statement

(Note: As of my knowledge cutoff in October 2023, LeetCode Problem #2908 does not exist. 
...
def containsNearbyDuplicate(nums, k):
    """
    Determines if there exists a pair of distinct indices (i, j) such that:
    1. nums[i] == nums[j]
    2. abs(i - j) <= k
...
- Therefore, the time complexity is O(n), where n is the length of the list `nums`.

Space Complexity:
- The function uses a dictionary `last_seen` to store the last seen index of each number.
- In the worst case, the dictionary could store up to n entries (if all elements in `nums` are unique).
