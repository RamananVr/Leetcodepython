"""
LeetCode Question #2604: Minimum Time to Eat All Grains

Problem Statement:
You are given an array `piles` where `piles[i]` represents the number of grains in the i-th pile, 
and an integer `h` representing the maximum number of hours you have to eat all the grains. 
You can eat grains at a constant speed `k` grains per hour. If you start eating from a pile, 
you must finish eating that pile before moving to the next one. You cannot eat from multiple piles simultaneously.
...
if __name__ == "__main__":
    ...
    piles = [30, 11, 23, 4, 20]
    h = 5
    print(minEatingSpeed(piles, h))  # Output: 30
    ...
    piles = [1, 1, 1, 1]
    h = 4
    print(minEatingSpeed(piles, h))  # Output: 1
