...
def myPow(x: float, n: int) -> float:
    """
    Calculate x raised to the power n (x^n) using an optimized approach.
    """
    if n == 0:
...
"""
Time Complexity:
- O(log n): The algorithm reduces the power `n` by half in each iteration, leading to a logarithmic number of steps.

Space Complexity:
- O(1): The algorithm uses a constant amount of extra space.

Topic: Math, Exponentiation by Squaring
"""