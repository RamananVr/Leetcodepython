"""
LeetCode Problem #2985: Check if a String is a Prefix of Array
...
- In the worst case, we iterate through all the strings in `words` and concatenate them until the length of the prefix exceeds `s`.
- The total number of characters processed is proportional to `m`.
- Therefore, the time complexity is O(m).

Space Complexity Analysis:
- The space complexity is O(n) because we are building the `prefix` string, which can grow up to the length of `s`.
